<script lang="ts">
  ...
  const query = useInfiniteQuery(activityHistoryQuery({ slug, limit: 30, page: 1 }));
  const items = $derived(
        ...
        <li class="profile-activity__row">
          ...
          <div class="profile-activity__body">
            ...
            <a class="profile-activity__title" href={target.href}>
              {target.title}
              {#if target.year}
                <span class="profile-activity__year">{target.year}</span>
              {/if}
              {#if episodeLabel}
                <span class="profile-activity__episode">{episodeLabel}</span>
              {/if}
            </a>
