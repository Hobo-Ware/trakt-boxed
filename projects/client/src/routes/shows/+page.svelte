<script lang="ts">
  /*
    Shows catalog. Sibling to /movies — Letterboxd has no shows
    surface, so we extend the same Fraunces editorial header + four
    stacked poster grids treatment. Each section pulls its own
    query (showTrending / showAnticipated / showPopular).
  */
  ...
  import { showTrendingQuery } from "$lib/requests/queries/shows/showTrendingQuery.ts";
...
>
  ...
  <div class="lb-cat-page">
    <header class="lb-cat-hero">
      <p class="lb-cat-hero-eyebrow">{m.lb_catalog_eyebrow_shows()}</p>
      <h1 class="lb-cat-hero-title">{m.button_label_shows()}</h1>
      <p class="lb-cat-hero-lede">{m.lb_catalog_lede_shows()}</p>
    </header>

    <LetterboxdCatalogSection
      label={m.lb_catalog_section_popular_this_week()}
      drilldownHref={UrlBuilder.popular({ mode: "show" })}
      query={showTrendingQuery({ limit: 12 })}
      limit={12}
    />

    <LetterboxdCatalogSection
      label={m.lb_catalog_section_most_anticipated()}
      drilldownHref={UrlBuilder.anticipated({ mode: "show" })}
      query={showAnticipatedQuery({ limit: 12 })}
      limit={12}
    />
    ...
    <LetterboxdCatalogSection
      ...
      drilldownHref={UrlBuilder.popular({ mode: "show" })}
      query={showPopularQuery({ limit: 12 })}
      limit={12}
    />
    ...
    {#if $isAuthorized}
      <LetterboxdCatalogSection
        ...
        drilldownHref={UrlBuilder.recommended({ mode: "show" })}
        query={showTrendingQuery({ limit: 12 })}
        limit={12}
      />
    {/if}
